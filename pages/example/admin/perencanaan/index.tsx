...
import {
  EyeIcon, PlusIcon, PencilIcon as EditIcon, TrashIcon,
} from '@heroicons/react/24/solid';
import Layout from 'example/containers/Layout';
import PageTitle from 'example/components/Typography/PageTitle';

import AddBudgetModal from '../perencanaan/tambah';
import EditBudgetModal from '../perencanaan/edit';
import DetailBudgetModal from '../perencanaan/detail';
import DeleteBudgetModal from '../perencanaan/delete';
...
const initialBudgets: Budget[] = [
  { id: 1, name: 'Gaji Karyawan', amount: 4000000, spent: 4000000,  Date: new Date('2025-03-01') },
  { id: 2, name: 'Listrik', amount: 1000000, spent: 950000,  Date: new Date('2025-03-01') },
  { id: 3, name: 'PDAM', amount: 1000000, spent: 800000,  Date: new Date('2025-03-01') },
  { id: 4, name: 'Kebutuhan Lapangan', amount: 500000, spent: 600000,  Date: new Date('2025-03-01')},
  { id: 5, name: 'Kebutuhan Lainnya', amount: 500000, spent: 0, Date: new Date('2025-03-01') },
];
    ...
    <Layout>
      ...
      <div className="bg-white px-6 py-4 rounded-lg shadow-md inline-flex min-w-[300px] max-w-fit h-[80px] items-center">
        ...
        <h3 className="text-xl font-semibold">
          Total Anggaran Direncanakan: Rp {totalPlanned > 0 ? totalPlanned.toLocaleString('id-ID') : '-'}
        </h3>
      </div>
      ...
      <div className="bg-white px-6 py-4 rounded-lg shadow-md inline-flex min-w-[300px] max-w-fit h-[80px] items-center">
        ...
        <h3 className="text-xl font-semibold">
          Total Pengeluaran: Rp {totalSpent > 0 ? totalSpent.toLocaleString('id-ID') : '-'}
        </h3>
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {paginated.map((b, idx) => {
                ...
                const persentase = b.amount > 0 ? ((selisih / b.amount) * 100).toFixed(2) : '0.00';
                return (
                  <TableRow key={b.id}>
                    <TableCell>{idx + 1 + (page - 1) * resultsPerPage}</TableCell>
                    ...
                    <TableCell>
                      <div className="flex space-x-2">
                        <Button size="small" className="bg-blue-700 text-white" onClick={() => setSelected(b)}>
                          <EyeIcon className="w-4 h-4 mr-1" /> Lihat
                        </Button>
                        <Button size="small" className="bg-yellow-400 text-black hover:bg-yellow-500" onClick={() => setEditing(b)}>
                          <EditIcon className="w-4 h-4 mr-1" /> Edit
                        </Button>
                        <Button size="small" className="bg-red-600 text-white hover:bg-red-700" onClick={() => setDeleting(b)}>
